...
<head>
  ...
  <title>Add Mood Journal Entry — Silent Echoes</title>
...
<body>
  ...
  <div class="d-flex justify-content-center align-items-start py-5">
    <div class="notebook-card">
      <h2 class="text-center mb-4 notebook-title">Add Mood Journal Entry</h2>

      <form action="{{ route('mood-journals.store') }}" method="POST">
        @csrf
        ...
        <div class="mb-3">
          ...
          <div class="d-flex align-items-center">
            <input type="text" name="mood" id="mood" class="form-control me-2" value="{{ old('mood') }}" required>
        ...
        <div class="mb-3">
          ...
          <div id="editor" style="height:200px;">{{ old('entry') }}</div>
        ...
        <div class="mb-3">
          ...
          <input type="datetime-local" name="entry_date" class="form-control" value="{{ old('entry_date') }}" required>
          @error('entry_date') <small class="text-danger">{{ $message }}</small> @enderror
        </div>

        <div class="d-flex justify-content-between mt-4">
          <button type="submit" class="btn btn-mood">Save</button>
  ...
  <script>
    ...
    form.onsubmit = function() {
      var entry = document.querySelector('input[name=entry]');
      entry.value = quill.root.innerHTML;
    };
